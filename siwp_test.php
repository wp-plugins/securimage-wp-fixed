...
Fixed and improved by Jehy http://jehy.ru/articles/
		...
		if (isset($_GET['tested'])) {
			if (!isset($_SESSION['securimage_test_value'])) {
				$GLOBALS['session_start_error'] =
				   ...
				   ."was not found.<br />Click <a href=\"".htmlspecialchars($_SERVER['PHP_SELF'])."\">"
			...
			header('Location: ' . htmlspecialchars($_SERVER['PHP_SELF']) . '?tested=1');
			exit;
		}
	}
}
